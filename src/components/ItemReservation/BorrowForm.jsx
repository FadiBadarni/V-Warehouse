...
const BorrowForm = ({
  ...
  const handleStartDateChange = async (date) => {
    ...
    const result = await getEveryTimeSchedule(selectedInstanceIds,date,itemIdValue)
    ...
    for (let hour = 0; hour < 24; hour++) {
      for (let minute = 0; minute < 60; minute += 30) {
        timeSlots.push(datestr+"T"+hour.toString().padStart(2, '0')+":"+minute.toString().padStart(2,'0'));
      }
    }
...
const x=result.bendingStartDates;
  ...
  const handleReturnDateChange = async (date) => {
    ...
    if (startTime.hasOwnProperty(dateTimeString)) {
      ...
      const result = await getEveryTimeToReturnInSchedule(selectedInstanceIds, startDate, date,itemIdValue, x);
    ...
    const datestr=originalDate.toISOString().substring(0, 10);
      ...
      for (let hour = 0; hour < 24; hour++) {
        for (let minute = 0; minute < 60; minute += 30) {
          timeSlots.push(datestr+"T"+hour.toString().padStart(2, '0')+":"+minute.toString().padStart(2,'0')+':00');
        }
      }
  
      const newList = timeSlots.filter((time) => ! listforAllReturnData.includes(time));
      console.log(newList);
